...
use crate::{JsonClient, SlackError, SlackResult, Conversation, User, Message};
...
impl SlackClient {
    ...
    #[cfg(test)]
    pub(crate) fn new_with_json_client(json_client: JsonClient) -> Self {
        Self { json_client }
    }

    pub fn list_conversations(&self) -> SlackResult<Vec<Conversation>> {
        use crate::conversation::meta::list::{Response, METHOD};
    ...
    pub fn find_conversation(&self, conversation_id: &str) -> SlackResult<Conversation> {
        use crate::conversation::meta::find::{Response, METHOD};
    ...
    pub fn conversation_history(&self, conversation_id: &str) -> SlackResult<Vec<Message>> {
        use crate::conversation::meta::history::{Response, METHOD};
        let method = format!("{}?channel={}", METHOD, conversation_id);
        self.json_client.get_json::<Response>(&method)?.to_result()
    }

    pub fn list_users(&self) -> SlackResult<Vec<User>> {
        use crate::user::meta::list::{Response as ListUsersResponse, METHOD as LIST_USERS};
